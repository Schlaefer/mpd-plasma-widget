...
Item {
    ...
    function reconnect() {
        mpdRootExecutable.sources.forEach((source) => {
            mpdRootExecutable.disconnectSource(source);
        });
        mpdRoot.update();
    }
    ...
    function getVolume() {
        mpdRootExecutable.exec("mpc --host=" + mpdRoot.mpdHost + " volume" + " #getVolume");
    }

    function setVolume(value) {
        mpdRootExecutable.exec("mpc --host=" + mpdRoot.mpdHost + " volume " + value + " #update");
    }
    ...
    function getInfo() {
        mpdRootExecutable.exec("mpc --host=" + mpdRoot.mpdHost + " -f '{[\\n\"artist\": \"%artist%\", ][\\n\"albumartist\": \"%albumartist%\", ][\\n\"album\": \"%album%\", ][\\n\"tracknumber\": \"%track%\", ]\\n\"title\": \"%title%\", [\\n\"date\": \"%date%\", ]\\n\"file\": \"%file%\"\\n}' | head -n -2 #getInfo");
    }

    function removeFromQueue(position) {
        mpdCommandQueue.add("mpc --host=" + mpdRoot.mpdHost + " del " + position);
    }
    ...
    function getQueue() {
        mpdRootExecutable.exec("mpc --host=" + mpdRoot.mpdHost + " playlist -f '\"%file%\": {[\\n\"artist\": \"%artist%\", ][\\n\"albumartist\": \"%albumartist%\", ][\\n\"album\": \"%album%\", ][\\n\"tracknumber\": \"%track%\", ]\\n\"title\": \"%title%\", [\\n\"date\": \"%date%\", ]\\n\"file\": \"%file%\",\\n\"position\": \"%position%\"},' #getQueue");
    }
    ...
    function playInQueue(title) {
        mpdCommandQueue.add("sleep 1 && mpc --host=" + mpdRoot.mpdHost + " play \"" + title + "\"");
    }

    function getPlaylists() {
        mpdRootExecutable.exec("mpc --host=" + mpdRoot.mpdHost + " lsplaylist #getPlaylists");
    }
    ...
    function getOptions() {
        mpdRootExecutable.exec("mpc --host=" + mpdRoot.mpdHost + " status '{\"consume\": \"%consume%\", \"random\": \"%random%\"}' #getOptions");
    }

    function toggleRandom() {
        let newState = mpdRoot.mpdOptions.random === "on" ? "off" : "on";
        mpdCommandQueue.add("mpc --host=" + mpdRoot.mpdHost + " random " + newState);
    }

    function toggleConsume() {
        let newState = mpdRoot.mpdOptions.consume === "on" ? "off" : "on";
        mpdCommandQueue.add("mpc --host=" + mpdRoot.mpdHost + " consume " + newState);
    }
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            mpdRootExecutable.exec('mpc --host=' + mpdRoot.mpdHost + ' idle player mixer playlist options #idleLoop');
        }
    }
